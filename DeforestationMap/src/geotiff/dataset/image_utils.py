...
def stack_temporal_images(files, output_img, band_names=None):
    ...
    outds = drv.Create(output_img, out_xSize, out_ySize, num_bands, datatype, options=['COMPRESS=LZW', 'BIGTIFF=YES'])
...
def mosaic_images(files, output_file, band_names=None):
    ...
    arguments = ['python','gdal_merge.py', '-o', output_file, '-co','COMPRESS=LZW', '-co', 'BIGTIFF=YES', '-q', '-v']
...
def clip_by_aggregated_polygons(in_raster_path, shape_file, output_path, band_names=None, no_data=None):
    ...
    with rasterio.open(in_raster_path) as src:
        out_image, out_transform = rasterio.mask.mask(src, features,  invert=True, nodata=no_data)
...
def clip_by_polygon(in_raster_path, geoms, output_path, band_names=None, no_data=None):
    ...
    with rasterio.open(in_raster_path) as src:
        out_image, out_transform = rasterio.mask.mask(src, geoms, invert=True, nodata=no_data)
        out_meta = src.meta.copy()
